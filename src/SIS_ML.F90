...
!> for both Forpy and pure Fortran, and can be found at https://github.com/William-gregory/SIS2/tree/forpy_SPEAR and
...
public :: ML_init,ML_inference,ML_final
...
type, public :: ML_CS ; private
  ...
  character(len=300)  :: CNN_script !< Torchscript for the CNN
  character(len=300)  :: ANN_script !< Torchscript for the ANN
  type(torch_model) :: CNN_model_ftorch !< ftorch model for CNN
  type(torch_model) :: ANN_model_ftorch !< ftorch model for ANN
  type(torch_tensor), dimension(1) :: IN_CNN_torch     !< Pointer to CNN inputs which are passed to PyTorch
  type(torch_tensor), dimension(1) :: IN_ANN_torch     !< Pointer to ANN inputs which are passed to PyTorch
  type(torch_tensor), dimension(1) :: dSIC_torch       !< Pointer to CNN predictions of aggregate SIC corrections
  type(torch_tensor), dimension(1) :: dCN_torch        !< Pointer to ANN predictions of category SIC corrections
...
subroutine ML_init(Time,G,param_file,diag,CS)
  ...
  call get_param(param_file, mdl, "CNN_SCRIPT", CS%CNN_script, &
      "Torchscript for the CNN which predicts dSIC", &
      default="/gpfs/f5/scratch/gfdl_o/William.Gregory/FTorch/Torchcripts/NetworkA_script.pt")

  call get_param(param_file, mdl, "ANN_SCRIPT", CS%ANN_script, &
...
subroutine ML_inference(IST, OSS, FIA, IOF, G, IG, ML, dt_slow)
  ...
  real(sp), dimension(1,8,SZIW_(ML),SZJW_(ML)) &
  ...
  dimX = SIZE(IN_CNN,3) - 2*ML%CNN_halo_size
  dimY = SIZE(IN_CNN,4) - 2*ML%CNN_halo_size
  allocate(IN_ANN(1,7,dimX,dimY))
  ...
  do j=jsdw,jedw ; do i=isdw,iedw
     if (G%mask2dT(i,j) == 1.0) then !is ocean
        IN_CNN(1,1,i,j) = (WH_SIC(i,j) - sic_mu)/sic_std
        IN_CNN(1,2,i,j) = (WH_SST(i,j) - sst_mu)/sst_std
        IN_CNN(1,3,i,j) = (WH_UI(i,j) - ui_mu)/ui_std
        IN_CNN(1,4,i,j) = (WH_VI(i,j) - vi_mu)/vi_std
        IN_CNN(1,5,i,j) = (WH_HI(i,j) - hi_mu)/hi_std
        IN_CNN(1,6,i,j) = (WH_TS(i,j) - ts_mu)/ts_std
        IN_CNN(1,7,i,j) = (WH_SSS(i,j) - sss_mu)/sss_std
        IN_CNN(1,8,i,j) = WH_mask(i,j)
     endif
  enddo; enddo

  !Call FTorch for CNN inference
  dSIC = 0.0
  call torch_tensor_from_array(ML%IN_CNN_torch(1), IN_CNN, in_layout, torch_kCPU)
  call torch_tensor_from_array(ML%dSIC_torch(1), dSIC, out_layout, torch_kCPU)
  call torch_model_forward(ML%CNN_model_ftorch, ML%IN_CNN_torch, ML%dSIC_torch)

  IN_ANN = 0.0
  do j=js,je ; do i=is,ie
     iT = i-ML%CNN_halo_size
     jT = j-ML%CNN_halo_size
     if (G%mask2dT(i,j) == 1.0) then !is ocean
        IN_ANN(1,1,iT,jT) = (dSIC(1,1,iT,jT) - dsic_mu)/dsic_std
        IN_ANN(1,2,iT,jT) = (IST%part_size(i,j,1) - cn1_mu)/cn1_std
        IN_ANN(1,3,iT,jT) = (IST%part_size(i,j,2) - cn2_mu)/cn2_std
        IN_ANN(1,4,iT,jT) = (IST%part_size(i,j,3) - cn3_mu)/cn3_std
        IN_ANN(1,5,iT,jT) = (IST%part_size(i,j,4) - cn4_mu)/cn4_std
        IN_ANN(1,6,iT,jT) = (IST%part_size(i,j,5) - cn5_mu)/cn5_std
        IN_ANN(1,7,iT,jT) = G%mask2dT(i,j)
     endif
     do k=1,SIZE(IN_ANN,2)
        if (is_NaN(real(IN_ANN(1,k,iT,jT), kind(posterior)))) then
           IN_ANN(1,k,iT,jT) = 0.0
        endif
     enddo
  enddo; enddo

  call torch_delete(ML%IN_CNN_torch)
  call torch_delete(ML%dSIC_torch)

  !Call FTorch for ANN inference
  dCN = 0.0
  call torch_tensor_from_array(ML%IN_ANN_torch(1), IN_ANN, in_layout, torch_kCPU)
  call torch_tensor_from_array(ML%dCN_torch(1), dCN, out_layout, torch_kCPU)
  call torch_model_forward(ML%ANN_model_ftorch, ML%IN_ANN_torch, ML%dCN_torch)
  ...
  do j=js,je ; do i=is,ie
     ...
     do k=1,ncat
        if (G%mask2dT(i,j) == 1.0) then !is ocean
           ...
           IST%dCN(i,j,k) = dCN(1,k,iT,jT)/(432000.0/dt_slow) !Network was trained on 5-day (432000-second) increments
        endif
  ...
  call torch_delete(ML%IN_ANN_torch)
  call torch_delete(ML%dCN_torch)
  deallocate(IN_ANN)
  deallocate(dCN)
  deallocate(dSIC)
...
subroutine ML_final(CS)
  type(ML_CS), intent(inout) :: CS
  call torch_delete(CS%CNN_model_ftorch)
  call torch_delete(CS%ANN_model_ftorch)
