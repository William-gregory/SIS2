...
subroutine ML_inference(Ice) !WG
  ...
  dt_slow = time_type_to_real(Ice%sCS%Time_step_slow)
  scale = Ice%sCS%ML_CSp%ML_freq/432000.0 !Network was trained on 5-day (432000-second) increments
  ...
  call enable_SIS_averaging(dt_slow, Ice%sCS%Time, Ice%sCS%ML_CSp%diag)
